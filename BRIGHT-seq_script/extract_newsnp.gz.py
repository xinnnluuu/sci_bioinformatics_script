# Script: extract_newsnp.gz.py
# Description: Extract and filter SNP (Single Nucleotide Polymorphism) data from a gzipped input file
# Usage: python extract_newsnp.gz.py <input.gz> <output.txt>
# This script processes SNP data and filters based on specific criteria:
# - Mutation frequency > 5%
# - At least 5 supporting reads for alternative bases
# Output format: chromosome, position, reference base, base counts
...
for line in infile:
    ...
    # Skip if reference base is N (ambiguous base)
    if info[2] == "N":
        continue
    
    # Filter condition: mutation frequency > 5%
    if float(info[8]) > 0.05:
        ...
        # Check SNP type and supporting reads
        # For each reference base, sum the counts of alternative bases
        # Write to output if at least 5 supporting reads for alternatives
